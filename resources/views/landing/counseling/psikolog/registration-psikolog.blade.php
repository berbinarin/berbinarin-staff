...
@section('content')
    <div
        ...
        <div id="modal"
            ...
            <div
                class="h-auto max-h-screen w-[70%] overflow-y-auto rounded-2xl bg-white p-6 shadow-md max-lg:h-[90%] max-sm:max-h-[90%] max-sm:w-[86%] max-sm:px-2">
                <h1
                ...
                <div class="mb-6">
                    ...
                    <ol class="mt-1 list-decimal space-y-1 pl-7">
                        <ul class="max-sm:text-sm">
                            Sesuai dengan ketentuan yang ada di kontrak kerja terkait pemberian benefit kepada staff yang
                            bekerja di PT. Berbinar Insightful Indonesia, bagi para staff yang ingin mengambil benefit
                            konseling 1 kali/bulan bersama psikolog (Pak Danny) dan Peer, kalian dapat mengikuti alur
                            dibawah ini
                        </ul>
                    </ol>
                </div>
                ...
                <div class="mt-4 flex justify-center lg:gap-x-3">
                    <button id="closeModal"
                        class="w-[90%] rounded-xl border-[1.5px] border-[#225062] bg-transparent bg-gradient-to-r from-[#3986A3] to-[#15323D] px-4 py-1.5 font-medium text-white max-sm:text-[15px] lg:w-1/4">Saya
                        Mengerti</button>
                </div>
            </div>
        </div>
        ...
        <form id="singleForm" action="{{ route('counseling.storePsikologStaffRegistration') }}" method="POST" class="flex flex-col" enctype="multipart/form-data">
            @csrf
    ...
    <script>
        ...
        document.getElementById('singleForm').addEventListener('submit', function(event) {
            const requiredFields = [
                ...
                'nama', 'email', 'tanggal_Lahir', 'tempat_lahir', 'alamat', 'umur', 'agama',
                'status_pernikahan',
