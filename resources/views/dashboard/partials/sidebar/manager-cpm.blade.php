...
@php
    $isActivePsikolog = Str::startsWith(Route::currentRouteName(), 'dashboard.psikolog-staff');
    $isActivePeer = Str::startsWith(Route::currentRouteName(), 'dashboard.peer-staff');
@endphp

<li class="my-5 rounded-lg p-2 {{ isRouteNameStartWith('dashboard.psikolog-staff', 'bg') }}">
    <a href="{{ route('dashboard.psikolog-staff.index') }}"
        class="flex flex-row items-center duration-700 {{ isRouteNameStartWith('dashboard.psikolog-staff') }}">
        <img src="{{ asset('assets/images/dashboard/icons/psikolog-dashboard.webp') }}" alt="Psikolog Icon"
            class="mr-2 h-[21px] w-[21px] object-contain transition duration-300 {{ $isActivePsikolog ? 'brightness-0 invert' : 'brightness-100' }} {{ isRouteNameStartWith('dashboard.psikolog-staff') }}" />
...
<li class="my-5 rounded-lg p-2 {{ isRouteNameStartWith('dashboard.peer-staff', 'bg') }}">
    <a href="{{ route('dashboard.peer-staff.index') }}"
        ...
        <img src="{{ asset('assets/images/dashboard/icons/people-dashboard.webp') }}" alt="Peer Counselor Icon"
